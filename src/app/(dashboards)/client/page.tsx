...
export default function ClientDashboard() {
  ...
  }, [studentList, studentNameFilter, studentClassFilter]);
  ...
  const renderStaffTable = () => (
    <Card id="report-table">
      <CardHeader>
        <CardTitle>Staff Attendance - {selectedDate ? format(selectedDate, 'PPP') : 'No date selected'}</CardTitle>
        <CardDescription>An overview of staff attendance for the selected date.</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {!staffInitialized ? (
              ...
              filteredStaffList.map((employee) => {
                const todaysAttendance = employee.attendanceStatus?.date === selectedDateStr ? employee.attendanceStatus : null;
                return (
                  <TableRow key={employee.id}>
                    <TableCell className="font-medium">{employee.name}</TableCell>
                    <TableCell>{employee.department}</TableCell>
                    <TableCell>{todaysAttendance?.inTime ? <Badge variant="default" className="bg-green-600 hover:bg-green-700">{todaysAttendance.inTime}</Badge> : <Badge variant="secondary">Not Logged</Badge>}</TableCell>
                    <TableCell>{todaysAttendance?.outTime ? <Badge variant="default" className="bg-blue-600 hover:bg-blue-700">{todaysAttendance.outTime}</Badge> : <Badge variant="secondary">Not Logged</Badge>}</TableCell>
                    <TableCell>{todaysAttendance?.totalHours ? <Badge variant="outline">{todaysAttendance.totalHours}</Badge> : <Badge variant="secondary">--</Badge>}</TableCell>
              ...
              <TableRow><TableCell colSpan={6} className="text-center h-24">No staff members match the current filters.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </CardContent>
  ...
  const renderStudentTable = () => (
     <Card id="report-table">
        <CardHeader>
            <CardTitle>Student Attendance - {selectedDate ? format(selectedDate, 'PPP') : 'No date selected'}</CardTitle>
            <CardDescription>An overview of student attendance for the selected date.</CardDescription>
        </CardHeader>
        <CardContent>
            <Table>
            <TableHeader>
            ...
            <TableBody>
                ...
                filteredStudentList.map((student) => {
                    const todaysAttendance = student.attendanceStatus?.date === selectedDateStr ? student.attendanceStatus : null;
                    return (
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between print-hide">
        ...
        <div className="flex gap-2">
           <Button variant="outline" onClick={handleGenerateCsv} disabled={!selectedDate || (activeTab === 'staff' && filteredStaffList.length === 0) || (activeTab === 'student' && filteredStudentList.length === 0)}>
