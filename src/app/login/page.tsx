  ...
  const onSubmit = async (values: z.infer<typeof loginFormSchema>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const destination = result.client?.isSetupComplete ? '/dashboards/client' : '/setup';
    ...
    <main className="flex flex-col items-center justify-center min-h-screen bg-background p-4 sm:p-8">
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardFooter className="flex justify-center text-sm">
            <p>Are you the admin? <Link href="/dashboards/super-admin/login" className="font-semibold text-primary hover:underline">Go to Super Admin</Link></p>
        </CardFooter>
      </Card>
    </main>
  );
